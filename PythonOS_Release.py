...
while running and loggedIn:
	cmd = input(f"{st.GRN + username()}@PythonOS{st.RESET}:{st.BLUE}~{st.RESET}{st.RED + symbol() + st.RESET if root else st.GRN + symbol() + st.RESET} ")
	cmd = cmd.lower()
	...
	if cmd == "help" or cmd == "list cmd":
		print(
			st.CYAN +
			"Commands:\n"
			"\t1. su = switch user to root user / switch back\n"
		...
		if root:
			...
			print(f"{st.RED + nameOfUser + st.RESET} as user {st.BLUE + username() + st.RESET}")
	elif cmd == "ping":
		pingaddress = input("Enter address to ping: ")
		pingcmd = f"ping {pingaddress}"
		os.system(pingcmd)
	elif cmd == "sysinfo": #*System info
		systeminfo()
	elif cmd == "shutdown -now":
		CLS()
		print(st.CYAN + "Made by Daniel Kristensen\nWritten in Python" + st.RESET)
		time.sleep(2)
		exit()
		...
		nameOfUser = input("Enter new name: ")
		nameOfUser = nameOfUser.title()
	else:
